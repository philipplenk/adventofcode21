...
int main(int argc, char* argv[])
{
	...
	to_visit.push_back({"start",{},false});
	...
	while(!to_visit.empty())
	{
		...
		else
		{
			...
			{
				if(n!="start" && (std::isupper(n.front()) || (visited.count(n)<1 || !next.joker_used)))
				{
					to_visit.push_back({n,visited,next.joker_used || (std::islower(n.front()) && visited.count(n)>=1)});
				}
			}
		}
	}
